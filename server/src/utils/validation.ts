...
export const commandSchema = z.object({
  ...
  currency: z.string().regex(/^[A-Z]{2,10}$/).optional(),
  recipient: z.string().regex(/^[a-zA-Z0-9_]{1,15}$/).optional(),
});

export const validateTweet = (tweet: any) => {
  return tweetSchema.safeParse(tweet);
};

export const validateCommand = (command: any) => {
