import { pollMentions, BOT_USERNAME } from './utils/pool';
import { logger } from './utils/logger';
import cron from 'node-cron';

const CRON_SCHEDULE = process.env.CRON_SCHEDULE || '*/5 * * * *'; // every 5 min

logger.info(`Starting Twitter bot for @${BOT_USERNAME} with schedule: ${CRON_SCHEDULE}`);
...
const isMainModule = process.argv[1]?.endsWith('bot.ts') || process.argv[1]?.endsWith('bot.js');
