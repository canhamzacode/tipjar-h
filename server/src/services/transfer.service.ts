...
export async function validateReceiver(receiverHandle: string): Promise<ReceiverValidationResult> {
...
export async function createPendingTip(transferRequest: TransferRequest): Promise<string> {
  logger.info("Creating pending tip", transferRequest);

  const receiverValidation = await validateReceiver(transferRequest.receiverHandle);
...
export async function createDirectTransaction(
  ...
  logger.info("Creating direct transaction", { ...transferRequest, receiverId });
...
export async function processTransferRequest(transferRequest: TransferRequest): Promise<TransferResult> {
